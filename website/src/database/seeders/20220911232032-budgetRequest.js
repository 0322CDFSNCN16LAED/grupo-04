  ...
  async up (queryInterface, Sequelize) {
    ...
    Array(20)
      ...
      .forEach((_, i) => {
        ...
        const randomUrgTrabajo = UrgTrabajoArray[Math.floor(Math.random() * UrgTrabajoArray.length)];
        const randomImg = ReqImgsArray[Math.floor(Math.random() * ReqImgsArray.length)];

        const randomBudgReq = {
          id: i + 1,
          tituloSolicitud: faker.lorem.words(5),
          userId: randomUserId,
          rubroNombre: randomRubro,
          detalleSolicitud: faker.lorem.paragraphs(3),
          urgenciaTrabajo: randomUrgTrabajo,
          ubicacion: faker.address.streetAddress() + ", " + faker.address.city() + ", " + faker.address.state(),
